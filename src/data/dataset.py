import operator
from collections import OrderedDict
from pathlib import Path

import numpy as np
import torch
from torch_geometric.data import Data

from . import shards

__all__ = ["JetDataset"]


class JetDataset(torch.utils.data.Dataset):
    """Jet graph dataset over sharded .pt files; each item is one jet's PyG Data.

    Shards load on demand into an LRU cache (max_cache in memory).

    Args:
        shard_dir:    dir from build_graph.py (shard_NNNN.pt + metadata.pt)
        max_cache:    shards held in the LRU cache (~136 MB each, 32 GB RAM)
    """

    def __init__(self, shard_dir: str, max_cache: int = 4):
        max_cache = operator.index(max_cache)
        if max_cache < 0:
            raise ValueError(f"max_cache must be non-negative, got {max_cache}")
        self.shard_dir = Path(shard_dir)
        self.meta = shards.load_metadata(self.shard_dir)
        self.labels = self.meta["labels"]  # (n,) int64
        self.event_ids = self.meta["event_ids"]
        self.jet_idx = self.meta["jet_idx"]
        self.shard_size = self.meta["shard_size"]
        self.n_shards = self.meta["n_shards"]
        self.max_cache = max_cache
        self._cache: OrderedDict = OrderedDict()

    def __len__(self) -> int:
        return self.meta["n_jets"]

    def __getitem__(self, idx: int) -> Data:
        idx = operator.index(idx)
        if idx < 0:
            idx += len(self)
        if idx < 0 or idx >= len(self):
            raise IndexError(
                f"JetDataset index {idx} is out of range for {len(self)} jets"
            )
        shard_idx, local_idx = divmod(idx, self.shard_size)
        return self.load_shard(shard_idx)[local_idx]

    def load_shard(self, shard_idx: int) -> list:
        shard_idx = operator.index(shard_idx)
        if shard_idx < 0 or shard_idx >= self.n_shards:
            raise IndexError(
                f"shard index {shard_idx} is out of range for "
                f"{self.n_shards} shards"
            )
        if self.max_cache == 0:
            return shards.load_shard(self.shard_dir, shard_idx)
        if shard_idx not in self._cache:
            self._cache[shard_idx] = shards.load_shard(self.shard_dir,
                                                       shard_idx)
            if len(self._cache) > self.max_cache:
                self._cache.popitem(last=False)
        else:
            self._cache.move_to_end(shard_idx)
        return self._cache[shard_idx]

    def background_indices(self) -> np.ndarray:
        """Indices of background jets (label == 0)."""
        return np.where(self.labels == 0)[0]

    def stats(self) -> dict:
        out = {
            "n_jets": self.meta["n_jets"],
            "n_events": self.meta["n_events"],
            "n_signal": int(self.labels.sum()),
            "n_background": int((self.labels == 0).sum()),
            "mean_nodes": float(self.meta["n_nodes"].mean()),
        }
        if "n_edges" in self.meta:  # absent for point-cloud (step-1)
            out["mean_edges"] = float(self.meta["n_edges"].mean())
        return out

    def __repr__(self) -> str:
        edges = self.meta.get("edges") or {}
        nodes = self.meta["nodes"]
        return (f"JetDataset(n={len(self):,}, shards={self.n_shards}, "
                f"strategy={edges.get('strategy', 'none')!r}, "
                f"features={nodes['features']!r}, "
                f"dir={self.shard_dir.name})")
